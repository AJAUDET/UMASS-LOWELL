...
class EDistance {
 public:
  ...
  void hirschberg(const std::string& a, int aStart, int aEnd,
                  const std::string& b, int bStart, int bEnd,
                  std::stringstream& out);
  std::vector<int> computeForward(const std::string& a, int aStart, int aEnd,
                                  const std::string& b, int bStart, int bEnd);
  std::vector<int> computeBackward(const std::string& a, int aStart, int aEnd,
                                   const std::string& b, int bStart, int bEnd);

  static int customMin(int a, int b, int c) {
    if (a <= b && a <= c) return a;
    if (b <= a && b <= c) return b;
    return c;
  }
