...
public:
    ...
    // can directly access private variables
    friend Money &operator++(Money &operand);
    friend const Money operator++(Money operand, int);

    const Money operator+(const Money& operand);
